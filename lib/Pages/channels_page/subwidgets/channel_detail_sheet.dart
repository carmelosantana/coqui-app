...
class _ChannelDetailSheetState extends State<ChannelDetailSheet> {
  ...
  Future<void> _refresh({bool force = false}) async {
    ...
    await context.read<ChannelProvider>().loadChannelDetail(widget.channel.id, force: force);
    if (mounted) {
  ...
  Future<void> _toggleEnabled(CoquiChannel channel) async {
    final provider = context.read<ChannelProvider>();
    final updated = await provider.toggleChannelEnabled(channel.id, !channel.enabled);
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
  ...
  Future<void> _deleteLink(CoquiChannel channel, CoquiChannelLink link) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('This removes ${link.remoteUserKey} from ${channel.displayName}.'),
        actions: [
          TextButton(
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          success ? 'Linked sender removed.' : provider.error ?? 'Unable to remove link',
        ),
      ),
    );
  }
  ...
  Future<void> _deleteChannel(CoquiChannel channel) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('The channel configuration will be removed from the Coqui server.'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context, false),
            child: const Text('Cancel'),
                  ...
                  child: Row(
                    children: [
                      ...
                      IconButton(
                        ...
                        onPressed: provider.isTesting(channel.id) ? null : _testChannel,
                      ),
                    ...
                    children: [
                      ...
                      Wrap(
                        ...
                        children: channel.capabilities.entries
                            ...
                            .map(
                              (entry) => Chip(label: Text(entry.key.replaceAll('_', ' '))),
                            )
                            .toList(),
                      ),
                      const SizedBox(height: 16),
                      ...
                      if (events.isEmpty)
                        ...
                        ...events.take(6).map((event) => _EventTile(event: event)),
                      const SizedBox(height: 16),
                      ...
                      if (deliveries.isEmpty)
                        ...
                        ...deliveries
                            .take(6)
                            .map((delivery) => _DeliveryTile(delivery: delivery)),
...
class _InfoGrid extends StatelessWidget {
  ...
  const _InfoGrid({required this.channel});
...
class _HintBlock extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final body = switch (channel.driver) {
      'signal' => 'Test Connection runs a reconcile and health refresh on the Coqui API server. For a real Signal check, verify signal-cli --version, java --version, and a valid attached account, then link a sender profile and send a real Signal message.',
      'telegram' || 'discord' => 'This driver is currently scaffolded in the backend. You can save configuration and monitor placeholder health, but end-to-end transport testing is not yet available.',
      _ => 'Use Test Connection to refresh runtime state and then inspect recent events and deliveries below.',
    };
      ...
      child: Row(
        ...
        children: [
          Icon(
            ...
            size: 18,
            color: failed ? Theme.of(context).colorScheme.error : CoquiColors.chart3,
          ),
          const SizedBox(width: 10),
          Expanded(
            child: Column(
